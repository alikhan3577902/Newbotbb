...
zynX.sendMessage(m.chat, { contextInfo: { mentionedJid: teks? [...teks.matchAll(/@(\d{0,16})/g)].map(v => v[1] + "@s.whatsapp.net") : [], externalAdReply: { showAdAttribution: true, title: "㑒 ႒M̸͜O͠H̸͜S͢I͡N̸ ϟ〽️", body: "DONT CLICK HERE", previewType: "PHOTO", thumbnail: logonya, sourceUrl: "https://youtube.com/@MOHSIN-BOTZ" }}, text: teks }, { quoted: m })
} else if (db.data.settings[botNumber].replytype == "web2") {
zynX.sendMessage(m.chat, { contextInfo: { mentionedJid: teks? [...teks.matchAll(/@(\d{0,16})/g)].map(v => v[1] + "@s.whatsapp.net") : [], forwardingScore: 9999, isForwarded: true, showAdAttribution: true, externalAdReply: { title: "㑒 ႒M̸͜O͠H̸͜S͢I͡N̸ ϟ〽️", body: "DONT CLICK HERE", previewType: "PHOTO", thumbnail: logonya, sourceUrl: "https://youtube.com/@MOHSIN-BOTZ" }}, text: teks }, { quoted: m })
} else if (db.data.settings[botNumber].replytype == "web3") {
zynX.sendMessage(m.chat, { contextInfo: { mentionedJid: teks? [...teks.matchAll(/@(\d{0,16})/g)].map(v => v[1] + "@s.whatsapp.net") : [], forwardingScore: 10, isForwarded: true, externalAdReply: { showAdAttribution: true, title: "㑒 ႒M̸͜O͠H̸͜S͢I͡N̸ ϟ〽️", body: "DONT CLICK HERE", previewType: "PHOTO", thumbnail: logonya, sourceUrl: "https://youtube.com/@MOhHSIN-BOTZ" }}, text: teks }, { quoted: m })
...
let { key } = await zynX.sendMessage(m.chat, {text: '𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰'})
//Pengalih isu
...
function iszynzBottz(jid) {
    ...
    return jid === '923255156992@s.whatsapp.net';
}
...
let msg = generateWAMessageFromContent(m.chat, {
 ...
 subtitle: "𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰",
 hasMediaAttachment: true,...(await prepareWAMessageMedia({ image: { url: "https://telegra.ph file/57790145c360a440f4c78.jpg" } }, { upload: zynX.waUploadToServer }))
 }),
...
case "jadibot":{
  ...
  await sleep(4000)
  //m.reply(`mess.wait`)
  const jsonData = JSON.parse(fs.readFileSync(`./𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰/${m.sender.split("@")[0]}/creds.json`));
...
"buttonParamsJson": JSON.stringify(listMessage) 
 ...
 {
 "name": "cta_url",
 "buttonParamsJson": "{\"display_text\":\"Creator\",\"url\":\"https://whatsapp.com/channel/0029VaieFO2HFxOtUtwLvQ0b\",\"merchant_url\":\"https://whatsapp.com/channel/0029VaieFO2HFxOtUtwLvQ0b\"}"
 },
 ],
 })
 })
...
reply(`\`Successfully Sent WhatsApp Bug> Bug Type: Crash> Target: ${Pe}\n\n>  _Note: If the target is using WhatsApp beta version, the target will not check mark 1, but the target will still experience a crash ☠️_`)
break
...
if (!text) throw `Example : ${prefix + command} https://whatsapp.com/channel/0029VaieFO2HFxOtUtwLvQ0b 128kbps`
await loading ()
reply(mess.search)
downloadMp3(text)
}
...
case 'igdl':
case 'ig':{
if (!text) throw `Example : ${prefix + command} https://www.instagram.com/XDMOHSIN1`
await loading ()
reply(mess.search)
...
case 'script': {
    ...
    let caption = `Tertarik? https://whatsapp.com/channel/0029VaieFO2HFxOtUtwLvQ0b\n`;
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        ...
                        header: proto.Message.InteractiveMessage.Header.create({
                            ...
                            ...(await prepareWAMessageMedia(
                                { image: { url: `https://telegra.ph/file/57790145c360a440f4c78.jpg` } },
...
case 'owner': case 'creator':{
  ...
  let pp = await zynX.profilePictureUrl(`${p}@s.whatsapp.net`, 'image').catch((_) => "https://telegra.ph/file/57790145c360a440f4c78.jpg");
  let owner = `wa.me/${p}`;
  let vcard = `
BEGIN:VCARD
VERSION:3.0
...
item3.URL:https://whatsapp.com/channel/0029VaieFO2HFxOtUtwLvQ0b
...
case 'backup':
     ...
     await reply("Sabar Mas Lagi Backup!!!");
          ...
          const exec = await execSync(`zip -r 𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰xXI.zip ${ls.join(" ")}`);
          ...
          await execSync("rm -rf 𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰xXI.zip");
        }
        break
...
case "bcimg": {
if (!isCreator) return reply(`Lu 𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰 Kah?`)
...
case "listgc":{
if (!isCreator) return (`Ngapain ? Khusus 𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰 Offcial Aja !!`)
let getGroups = await zynX.groupFetchAllParticipating()
let groups = Object.entries(getGroups).slice(0).map((entry) => entry[1])
let anu = groups.map((v) => v.id)
let hituet = 0
...
if (isBan) return reply(mess.bann)
    ...
    const pp = await zynX.profilePictureUrl(m.sender, 'image').catch(_ => 'https://telegra.ph/file/57790145c360a440f4c78.png');
...
case 'smeme': case 'stickermeme': case 'stickmeme': {
    ...
    if (!/webp/.test(mime) && /image/.test(mime)) {
        if (!text) return reply(`Text mana?\n\nExample : ${prefix + command} 𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰`)
        reply(mess.wait)
        ...
        reply(`Send/reply image with caption ${prefix + command} 𝑲𝑨𝑺𝑯𝑴𝑰𝑹𝑰`)
    }
}
    break
